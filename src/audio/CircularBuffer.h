...
template <typename T>
bool CircularBuffer<T>::write(const T* data, size_t count) 
{
    ...
    while ((capacity - available) < count && !is_closed) 
    {
...
template <typename T>
bool CircularBuffer<T>::isClosed() const 
{
    return is_closed;
}
